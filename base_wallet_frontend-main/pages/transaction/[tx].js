...
import { generateAddress } from "../../../base_wallet_backend-main/util/util.js";
export default function UnconfimredTx() {
  ...
  useEffect(() => {
    (async () => {
      console.log(`Raw Tx: ${TxId}`);
      if (TxId !== undefined) {
        const res = await fetch(
          `http://localhost:3000/api/fetchTx?TxId=${TxId}`
        );
        const newBlocks = await res.json();

        setBlockHeight(newBlocks[0].Height);
        setBlockhash(newBlocks[0].blockHeader.blockhash);
        console.log(`Transaction: ${newBlocks[0].blockHeader.blockhash}`);
        newBlocks[0].Transactions.forEach((tx) => {
          if (tx.TxId === TxId) {
            setTx(tx);
            console.log(tx);
          }
  ...
  if (Tx) {
    const { version, tx_ins, tx_outs, TxId, locktime } = Tx;
          ...
          <div className="bg-gradient-to-r  from-indigo-700 to-sky-600 rounded-2xl mx-[5em] overflow-hidden ">
            <div className=" grid grid-cols-[30%_70%] text-white px-[6em]">
              ...
              <div className="overflow-hidden">
                <div className="flex items-center h-[3em] text-lg border-b-[1px]">
                  <Link href={`/blocks/block?blockhash=` + Blockhash} passHref>
                    <a className="underline">{BlockHeight}</a>
                  </Link>
                </div>
                <div className="flex items-center h-[3em] text-lg border-b-[1px]">
                  <h1> Confirmed </h1>
                  ...
                  >
                    ...
                    {Buffer.from(transaction.prev_tx.data).toString("hex") !==
                      ...
                      <div className="pl-[0.3em]">
                        <Link
                          href={`/unconfirmedTxId/tx?TxId=${Buffer.from(
                            transaction.prev_tx
                          ).toString("hex")}`}
                          passHref
                        >
