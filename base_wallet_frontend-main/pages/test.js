...
export default function Blocks({ data }) {
  ...
  const loadLatestBlocks = async () => {
    ...
    // HeightRef.current = newBlocks[newBlocks.length - 1].Height;
    if (newBlocks[0].Height > LatestBlockHeight.current) {
  ...
  const lastBlockRef = useCallback(
    ...
    [] // isLoading.current is a ref and doesn't trigger re-renders, so it shouldn't be in dependencies
  );
            ...
            if (blocks.length === index + 1) {
              return (
                ...
                >
                  ...
                  <div className=" ">
                    <Link
                      href={`/block?blockhash=` + block.blockHeader.blockhash}
                      passHref
                    >
